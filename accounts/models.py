...
class CartItems(BaseModel):
    ...
    class Meta:
        verbose_name_plural="1. Cart items"
    def __str__(self):
        return str(self.cart.uid)+" user :"+str(self.cart.user) + " status: "+str(self.cart.is_paid)

    def get_product_gross_price(self):
        price=[self.product.price*self.quantity]

        if self.color_variant:
            color_variant_price=self.color_variant.price
            price.append(color_variant_price*self.quantity)
